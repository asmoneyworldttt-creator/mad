        ...
        <div className="animate-slide-up space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {kpiCards.map((card, idx) => (
                    <div key={idx} className={`p-8 rounded-[2rem] border relative overflow-hidden group hover:scale-[1.02] transition-all duration-500 ${isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-slate-100 shadow-sm'}`}>
                        ...
                        <p className={`text-[10px] font-extrabold uppercase tracking-[0.2em] mb-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{card.label}</p>
                ...
                <div className={`p-10 rounded-[3rem] border ${isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-slate-100 shadow-sm'}`}>
                                ...
                                <div className="flex justify-between items-center mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{doc.conversion}% Conversion</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-bold text-emerald-500">₹{(doc.revenue / 1000).toFixed(1)}k</p>
                                        <p className="text-[8px] text-slate-500 font-extrabold uppercase">Generated</p>
                                    </div>
                                </div>
                    ...
                    <div className="mt-10 p-6 rounded-2xl bg-amber-500/5 border border-amber-500/10">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h5 className="text-xs font-bold uppercase tracking-widest text-amber-500">Strategic Alert</h5>
                        </div>
            ...
            <div className={`p-10 rounded-[3rem] border ${isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-slate-100 shadow-sm'}`}>
                <div className="flex justify-between items-center mb-8">
                    ...
                    <button className="text-primary text-xs font-extrabold uppercase tracking-widest hover:underline">View Full Audit Trail</button>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-white/5">
                                <th className="pb-4 text-[10px] font-extrabold uppercase tracking-widest text-slate-500">Timestamp</th>
                                <th className="pb-4 text-[10px] font-extrabold uppercase tracking-widest text-slate-500">Entity</th>
                                <th className="pb-4 text-[10px] font-extrabold uppercase tracking-widest text-slate-500">Transaction</th>
                                <th className="pb-4 text-[10px] font-extrabold uppercase tracking-widest text-slate-500">Status</th>
                                <th className="pb-4 text-[10px] font-extrabold uppercase tracking-widest text-slate-500 text-right">Magnitude</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {[1, 2, 3, 4, 5].map((_, i) => (
                                <tr key={i} className="group hover:bg-white/5 transition-all">
                                    ...
                                    <td className="py-4">
                                        <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-full text-[9px] font-extrabold uppercase">Synced</span>
                                    </td>
