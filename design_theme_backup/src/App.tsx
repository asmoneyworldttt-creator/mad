  ...
  return (
    <div className={`min-h-screen flex ${theme === 'dark' ? 'text-white' : 'text-slate-900'} bg-[var(--bg-deep)] font-sans selection:bg-[var(--accent-cyan)]/30 ${isMobile ? 'pb-24 pt-safe-top' : ''} relative overflow-x-hidden`}>
      ...
      <div className={`flex-1 flex flex-col min-w-0 ${!isMobile ? 'lg:ml-64' : ''}`}>
        ...
        {isMobile && (
          <header className="sticky top-0 z-50 px-5 pt-4 pb-4 glass-morphism border-b border-white/10">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
                  <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-[var(--accent-cyan)]/30">
                    <img alt="Doctor" className="w-full h-full object-cover" src="https://images.unsplash.com/photo-1559839734-2b71ea197ec2?auto=format&fit=crop&q=80&w=150" />
                  </div>
                  <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 rounded-full border-2 border-[#0F171A]"></div>
                </div>
                <div>
                  <h1 className="font-display text-lg font-bold leading-none text-white">Dentora Space</h1>
                  <div className="flex items-center gap-1.5 mt-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent-cyan)] pulsing-dot"></span>
                    <span className="text-[10px] uppercase tracking-widest text-[var(--accent-cyan)] font-bold">4 Patients Waiting</span>
                  </div>
                </div>
              </div>
              <button onClick={() => switchRole(userRole === 'admin' ? 'patient' : 'admin')} className="w-10 h-10 rounded-xl glass-morphism flex items-center justify-center text-white/70 hover:text-white group">
