...
function StatCard({ title, value, change, trend, delay, onClick, icon }: any) {
    return (
        ...
        >
            <div className="absolute top-0 right-0 p-3 opacity-80 text-2xl emoji-shadow">{icon || 'Γ£¿'}</div>
    ...
    return (
        <div className="animate-slide-up space-y-8 pb-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
                <StatCard title="Today" value={stats.todayAppointments.toString()} change="LIVE QUEUE" trend="up" delay={0.1} onClick={() => setActiveTab && setActiveTab('appointments')} icon="≡ƒª╖" />
                <StatCard title="Visits" value={`${(stats.totalVisits / 1000).toFixed(1)}k`} change="TOTAL LOYALTY" trend="up" delay={0.15} onClick={() => setActiveTab && setActiveTab('patients')} icon="≡ƒ⌐║" />
                <StatCard title="Reports" value={stats.pendingReports.toString()} change="3 URGENT" trend="down" delay={0.2} icon="≡ƒö¼" />
                <StatCard title="Revenue" value={`Γé╣${(stats.totalRevenue / 1000).toFixed(0)}k`} change="+12% VS LY" trend="up" delay={0.25} onClick={() => setActiveTab && setActiveTab('earnings')} icon="≡ƒÆÄ" />
                <StatCard title="Feedback" value="4.9" change="EXCELLENT" trend="up" delay={0.3} icon="Γ£¿" />
            </div>
                    ...
                    <div className="bg-white/40 backdrop-blur-md rounded-[32px] p-6 border border-white/60">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button onClick={() => setIsSlotModalOpen(true)} className="frosted-card rounded-2xl p-5 flex flex-col items-center gap-3 hover:scale-105 transition-all group active:scale-95">
                                <span className="text-2xl group-hover:scale-110 transition-transform emoji-shadow">≡ƒôà</span>
                            ...
                            <button onClick={() => setIsPrescModalOpen(true)} className="frosted-card rounded-2xl p-5 flex flex-col items-center gap-3 hover:scale-105 transition-all group active:scale-95">
                                <span className="text-2xl group-hover:scale-110 transition-transform emoji-shadow">≡ƒô¥</span>
                            ...
                            <button onClick={() => showToast('AI Analysis coming soon', 'success')} className="frosted-card rounded-2xl p-5 flex flex-col items-center gap-3 hover:scale-105 transition-all group active:scale-95">
                                <span className="text-2xl group-hover:scale-110 transition-transform emoji-shadow">≡ƒª╖</span>
                            ...
                            <button onClick={() => showToast('Lab Sync starting...', 'success')} className="frosted-card rounded-2xl p-5 flex flex-col items-center gap-3 hover:scale-105 transition-all group active:scale-95">
                                <span className="text-2xl group-hover:scale-110 transition-transform emoji-shadow">≡ƒº¬</span>
                        ...
                        {[
                            ...
                            <div key={idx} className="p-4 border border-slate-200 rounded-xl bg-slate-50 flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">{item.phone} ΓÇó {item.reason}</p>
                                </div>
