                    ...
                    <div key={i} className={`flex items-center justify-between p-4 rounded-xl border-l-4 transition-colors glass ${p.status === 'Engaged' ? 'border-orange-400 bg-orange-50/10 shadow-sm' : p.status === 'Checked-In' ? 'border-green-500 bg-green-50/10' : 'border-slate-300'}`}>
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500 font-medium">{p.condition} ΓÇó {p.time}</p>
                            </div>
                        </div>
    ...
    return (
        <div className="animate-slide-up space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                ...
                <StatCard title="Payment Collection" value={`Γé╣${stats.paymentCollection.toLocaleString('en-IN')}`} change="Verified" trend="up" delay={0.35} onClick={() => setActiveTab && setActiveTab('earnings')} />
                <StatCard title="Professional Fee" value={`Γé╣${stats.profFee.toLocaleString('en-IN')}`} change="Calculated" trend="up" delay={0.4} onClick={() => setActiveTab && setActiveTab('earnings')} />
                <StatCard title="Total Expenses" value={`Γé╣${stats.expenses.toLocaleString('en-IN')}`} change="Budgeted" trend="down" delay={0.45} />
                <StatCard title="Pending Lab Reports" value={stats.pendingReports.toString()} change="3 Urgent" trend="down" delay={0.5} />
                <StatCard title="Total Revenue" value={`Γé╣${stats.totalRevenue.toLocaleString('en-IN')}`} change="Gross" trend="up" delay={0.55} onClick={() => setActiveTab && setActiveTab('earnings')} />
            </div>
