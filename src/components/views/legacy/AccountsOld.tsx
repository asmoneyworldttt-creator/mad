                            ...
                            <tbody className="divide-y divide-slate-100">
                                {incomes.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                                        ...
                                        <td className="p-4 text-sm font-bold text-text-dark">Γé╣{item.amount.toLocaleString()}</td>
                                        <td className="p-4 text-sm font-bold text-success">Γé╣{item.received_amount?.toLocaleString() || '0'}</td>
                                    </tr>
                                ))}
                                {incomes.length === 0 && (
                                    <tr>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {expenses.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                                        <td className="p-4 font-bold text-sm text-text-dark">{new Date(item.date).toLocaleDateString()}</td>
                                        <td className="p-4 text-sm font-medium text-slate-600">{item.category}</td>
                                        <td className="p-4 text-sm font-bold text-alert">Γé╣{item.amount.toLocaleString()}</td>
                                    </tr>
                                ))}
                                {expenses.length === 0 && (
                                    <tr>
